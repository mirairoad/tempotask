...
import { RedisConnection, CronJobType, JobData, JobEntry, JobOptions } from './types.ts';
...
export class JobQueueManager {
    ...
    static async init(db: RedisConnection, ctx: unknown = {}, concurrency: number = 1): Promise<JobQueueManager> {
      if (!JobQueueManager.instance) {
        JobQueueManager.instance = new JobQueueManager(db, ctx, concurrency);
      }
      await JobQueueManager.instance.createConsumerGroup();
      return JobQueueManager.instance;
    }
  
    async createConsumerGroup(): Promise<void> {
      try {
        await this.db.xgroup("CREATE", STREAM_NAME, CONSUMER_GROUP, "$", "MKSTREAM");
      } catch (err) {
